...
items_to_keep <- 15 #рекомендуем по 15 фильмов
rating_threshold <- 3
n_eval <-1
  
set.seed(100)
eval_sets <- evaluationScheme(data = ratings_movies, method = "split",
train = percentage_training, given = items_to_keep, goodRating = rating_threshold, k = n_eval) #случайным образом выбирает 15 фильмов видимыми
...
```{r}
model_to_evaluate <- "IBCF" #берет матрицу и смотрит, берет фильмы, кот нра и берет похожие на них фильмы. Суммирует. Если фильм похож на то, что нра - прибавляется плюсик. И наоборот может отбавляться. Другой метод - пользователи. Находим топ-10, топ-100 пользователей похожих, считается взвеш сумма.
...
```{r}
items_to_recommend <- 20 #предсказание 20 фильмов
...
TRUE) #считает правильность
head(eval_accuracy) #на сколько баллов ошибка, у 3 фигово, у 12 норм
```
...
```{r}
plot(list_results, annotate = 1, legend = "topleft") #ROC-кривая
```
